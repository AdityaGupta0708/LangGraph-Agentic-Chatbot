...
if uploaded_file:
    ...
    with st.spinner("Processing PDF..."):
        try:
            ...
            st.session_state.vector_store = FAISS.from_documents(chunks, embedding=embeddings)
...
if st.button("Submit"):
    with st.spinner("Generating answer..."):
        try:
            # Use existing vector store from session state
            answer = process_user_query(
                user_query, st.session_state.thread_id, st.session_state.vector_store
            )
